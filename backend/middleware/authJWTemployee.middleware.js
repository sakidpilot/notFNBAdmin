...
const authEmployeeMiddleware = (req, res, next) => {
    ...
    try {
        ...
        if(decodedAuth.role=="user")
        {
            console.log("Token auth not verified, user permission restricted!");
            return res.status(401).json({message: 'User permission not granted! Access Denied!'})
        }
        req.user = decodedAuth.user;
        console.log("Token auth verified, user permissions access granted!");
        next();
